...
public class JasperHelperForBAAL {
    public JasperPrint compileReportsAndThenPrint(String docType, String sourceData, Boolean isFcrRequired) throws JRException, UnsupportedEncodingException {
        ...
        if (isFcrRequired) {
            reportStream = getClass().getResourceAsStream("/reports/baal/KN_ESI_FCR_MASTER.jrxml");
            jasperReport = JasperCompileManager.compileReport(reportStream);
        } else {
            if (docType.equals("BL")) {
                reportStream = getClass().getResourceAsStream("/reports/baal/KN_ESI_BL_MASTER.jrxml");
                jasperReport = JasperCompileManager.compileReport(reportStream);
            } else if (docType.equals("FCR")) {
                reportStream = getClass().getResourceAsStream("/reports/baal/KN_ESI_FCR_MASTER.jrxml");
                jasperReport = JasperCompileManager.compileReport(reportStream);
            } else if (docType.equals("SWB")) {
                reportStream = getClass().getResourceAsStream("/reports/baal/KN_ESI_SWB_MASTER.jrxml");
                jasperReport = JasperCompileManager.compileReport(reportStream);
            } else if (docType.equals("RWB")) {
                reportStream = getClass().getResourceAsStream("/reports/baal/KN_ESI_RWB_MASTER.jrxml");
                jasperReport = JasperCompileManager.compileReport(reportStream);
            }
