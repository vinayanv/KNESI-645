...
@Slf4j
@Service
public class JasperHelperForLCL {
    public JasperPrint compileReportsAndThenPrint(String docType, String sourceData, Boolean isFcrRequired) throws JRException, UnsupportedEncodingException {
        ...
        if (isFcrRequired) {
            reportStream = getClass().getResourceAsStream("/reports/lcl/templates/KN_ESI_FCR_MASTER_LCL.jrxml");
            jasperReport = JasperCompileManager.compileReport(reportStream);
        } else {
            if (docType.equals("BL")) {
                reportStream = getClass().getResourceAsStream("/reports/lcl/templates/KN_ESI_BL_MASTER_LCL.jrxml");
                jasperReport = JasperCompileManager.compileReport(reportStream);
            } else if (docType.equals("FCR")) {
                reportStream = getClass().getResourceAsStream("/reports/lcl/templates/KN_ESI_FCR_MASTER_LCL.jrxml");
                jasperReport = JasperCompileManager.compileReport(reportStream);
            } else if (docType.equals("SWB")) {
                reportStream = getClass().getResourceAsStream("/reports/lcl/templates/KN_ESI_SWB_MASTER_LCL.jrxml");
                jasperReport = JasperCompileManager.compileReport(reportStream);
            } else if (docType.equals("RWB")) {
                reportStream = getClass().getResourceAsStream("/reports/lcl/templates/KN_ESI_RWB_MASTER_LCL.jrxml");
                jasperReport = JasperCompileManager.compileReport(reportStream);
            }
        }
    ...
    public String exportPdfFromTemplateForLCL(JasperPrint jasperPrint, String outputLocation) throws JRException {
